...
def config_one_plot(axes, fig, ind, to_plot, title):
...
def assess_solution(model):
    ...
    config_one_plot(axes0, fig0, 0, u_exact, 'Exact solution')
    config_one_plot(axes0, fig0, 1, u_pred, 'Predicted solution')
    config_one_plot(axes0, fig0, 2, diff_abs, 'Absolute difference')
    plt.tight_layout()
    plt.show()

    fig1, axes1 = plt.subplots(1, 3, figsize=(12, 5))
    config_one_plot(axes1, fig1, 0, du_exact_x1, 'Exact du/dx1')
    config_one_plot(axes1, fig1, 1, du_pred_x1, 'Predicted du/dx1')
    config_one_plot(axes1, fig1, 2, diff_abs_dx1, 'Absolute difference du/dx1')
    plt.tight_layout()
    plt.show()

    fig2, axes2 = plt.subplots(1, 3, figsize=(12, 5))
    config_one_plot(axes2, fig2, 0, du_exact_x2, 'Exact du/dx2')
    config_one_plot(axes2, fig2, 1, du_pred_x2, 'Predicted du/dx2')
    config_one_plot(axes2, fig2, 2, diff_abs_dx2, 'Absolute difference du/dx2')
    plt.tight_layout()
    plt.show()
    
if __name__=="__main__":
    model_name = 'ex2_res_iter20k_10k_2500_beta500.pth'
    model = rnn.RitzModel(2).to(('cpu'))
    model.load_state_dict(torch.load('./Ex2_singularity2D/Models/'+model_name, weights_only=True))
    print('\n')
    assess_solution(model)